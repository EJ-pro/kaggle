...
FEATURES = [
    ...
    'gender_code', 'family_size', 'family_type', # is_alone 제거
    'cabin_known', 'cabin_deck_code',
    # 'is_child', 'is_senior', 'age_bin',  <-- 이 3개 제거 (age_log로 충분함)
    'age_x_pclass', 'fare_x_pclass', 'gender_x_pclass',
    'ticket_group_size' # 'ticket_prefix_freq' 제거
]
...
BASE_MODELS = {
    ...
    'lgbm': LGBMClassifier(
          n_estimators=208,  learning_rate=0.01087339491356196,  max_depth=3,
          num_leaves=14,  subsample=0.5933091672480356,  colsample_bytree=0.7409166077471329,
...
submission.to_csv("submission_0310_3", index=False)
...
# 1. 메타 모델(Logistic Regression)의 각 베이스 모델별 가중치(Coefficient) 추출
model_names = list(BASE_MODELS.keys())
meta_weights = meta.coef_[0]

df_meta_weights = pd.DataFrame({
    'Model': model_names,
    'Weight': meta_weights
}).sort_values(by='Weight', ascending=False)

# 2. 트리 기반 베이스 모델들의 평균 피처 중요도 추출 (SVM은 제외)
tree_models = ['rf', 'et', 'gbm', 'xgb', 'lgbm']
avg_feature_importance = np.zeros(len(FEATURES))

for name in tree_models:
    # 마지막 Fold에서 학습된 모델의 중요도를 가져와 평균 계산
    avg_feature_importance += BASE_MODELS[name].feature_importances_ / len(tree_models)
...
# [그래프 1] 메타 모델 가중치
sns.barplot(x='Weight', y='Model', data=df_meta_weights, ax=axes[0], palette='viridis')
axes[0].set_title('Meta Model Weights\n(Which base model is trusted the most?)', fontsize=14)
axes[0].set_xlabel('Logistic Regression Coefficient (Weight)')
axes[0].set_ylabel('Base Models')

# [그래프 2] 피처 중요도
sns.barplot(x='Importance', y='Feature', data=df_features, ax=axes[1], palette='mako')
axes[1].set_title('Average Feature Importance\n(Across Tree-based Models)', fontsize=14)
