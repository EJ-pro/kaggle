...
def process_features(df):
    df = df.copy()
    df['fare'] = np.log1p(df.apply(lambda x: fare_map[x['pclass']] if pd.isna(x['fare']) else x['fare'], axis=1))
...
def add_te_features(df_tr, df_te, target_s):
    ...
    # 🌟 티켓 기준 타겟 인코딩 추가
    tr_t_surv, te_t_surv = apply_te_vectorized(df_tr, df_te, target_s, col='ticket', smoothing=2)
    df_tr['ticket_surv'] = tr_t_surv
    df_te['ticket_surv'] = te_t_surv
    
    return df_tr, df_te

features = ['pclass', 'age', 'fare', 'is_child', 'family_size', 'is_alone', 'title_code', 'pclass_sex', 'ticket_count', 'surname_surv', 'ticket_surv']
# 🌟 파라미터 max_depth 8로 증가
...
print("--- [Experiment Ver 3: Ticket TE 추가 및 트리 깊이(max_depth) 규제 완화] ---")
...
print(f"Ver 3 Train Score : {rf_model.score(X_tr, y_tr):.4f}")
print(f"Ver 3 Test Score  : {rf_model.score(X_te, y_te):.4f}\n")

importances = pd.Series(rf_model.feature_importances_, index=features)
print("[Ver 3 모델의 피처 중요도 (100점 만점)]")
print((importances * 100).sort_values(ascending=False).head(8))
print("\n" + "="*60 + "\n")

print("--- [Final Check: Stratified 5-Fold CV on Ver 3] ---")
