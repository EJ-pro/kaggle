...
features = ['pclass', 'age', 'fare', 'is_child', 'family_size', 'is_alone', 'title_code', 'pclass_sex', 'ticket_count', 'surname_surv']
...
print("--- [Experiment Ver 2: 파생변수 is_child 추가 및 Fare 로그 변환] ---")
...
print(f"Ver 2 Train Score : {rf_model.score(X_tr, y_tr):.4f}")
print(f"Ver 2 Test Score  : {rf_model.score(X_te, y_te):.4f}\n")

importances = pd.Series(rf_model.feature_importances_, index=features)
print("[Ver 2 모델의 피처 중요도 (100점 만점)]")
print((importances * 100).sort_values(ascending=False).head(8))
print("\n" + "="*60 + "\n")

print("--- [Final Check: Stratified 5-Fold CV on Ver 2] ---")
